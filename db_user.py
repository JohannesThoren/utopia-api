...
def create_user(mongo, username, password, email, image="http://localhost:3500/profilepic"):
      db = mongo.db.users
      
      if not db.find_one({"username":username}):
            password = bcrypt.hashpw(password.encode("UTF-8"), bcrypt.gensalt())
            new_user = {"username": username, "email": email ,"password": password, "token": uuid4(), "id": uuid4(), "image": image}
            db.insert(new_user)
