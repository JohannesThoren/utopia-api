...
def update_profile_helper(mongo, token, db, settings):
    if auth_user(mongo, token) != False:
        ...
        db.update({"username": username}, {"$set": {
                  "username": settings["username"], "info": settings["info"], "profile_picture": settings["profile_picture"]}})
        return {"msg": "settings changed", "response code": OK}
    else:
        return {"response code": NOT_FOUND, "err": "there are no user connected to your token"}
...
def update_profile_settings(mongo, token, settings):
    ...
    if not user:
        return update_profile_helper(mongo, token, db, settings)
    elif user["token"] == token:
        return update_profile_helper(mongo, token, db, settings)
    else:
        return {"err": "a user with that name already exists", "response code": NOT_ALLOWED}
...
def create_user(mongo, username, password, email, profile_picture="", info="No Info Yet"):
    ...
    if profile_picture == "":
        ...
        profile_picture = f"https://www.gravatar.com/avatar/{pp_hash}?s=1024&d=identicon"

    db = mongo.db.users
    if not db.find_one({"username": username, "email": email}):
        password = bcrypt.hashpw(password.encode("UTF-8"), bcrypt.gensalt())
        new_user = {"username": username, "email": email, "password": password, "info": info,
                    "token": secrets.token_urlsafe(64), "id": uuid4(), "profile_picture": profile_picture, "following": []}
        db.insert(new_user)
...
def get_user_by_token(mongo, token):
    ...
    if user:
        return {"response code": 200, "username": user["username"], "id": user["id"], "profile_picture": user["profile_picture"], "following": user["following"], "token":
                user["token"], "info": user["info"]}
    else:
        return {"response code": 404}
